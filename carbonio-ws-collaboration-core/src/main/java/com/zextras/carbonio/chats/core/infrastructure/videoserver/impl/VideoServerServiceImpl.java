...
import com.zextras.carbonio.chats.core.infrastructure.videoserver.data.request.audiobridge.AudioBridgeConfigureRequest;
...
import com.zextras.carbonio.chats.core.infrastructure.videoserver.data.request.videoroom.VideoRoomConfigureRequest;
...
public class VideoServerServiceImpl implements VideoServerService {
    ...
    public void startMeeting(String meetingId) {
        ...
        VideoServerResponse audioPluginResponse = attachToPlugin(
                connectionId,
                JANUS_AUDIOBRIDGE_PLUGIN,
                String.format(MEETING_AUDIO_OPAQUE_ID_PATTERN, meetingId));
        VideoServerResponse videoPluginResponse = attachToPlugin(
                connectionId,
        ...
        AudioBridgeResponse audioRoomResponse = createAudioBridgeRoom(meetingId, connectionId, audioHandleId);
    ...
    private VideoServerResponse attachToPlugin(
        ...
        VideoServerResponse response = interactWithConnection(connectionId, JANUS_ATTACH, pluginType, opaqueId);
        if (!JANUS_SUCCESS.equals(response.getStatus())) {
    ...
    private AudioBridgeResponse createAudioBridgeRoom(
        ...
        AudioBridgeCreateRequest audioRequest = AudioBridgeCreateRequest.create()
        ...
        AudioBridgeResponse response = sendAudioBridgePluginMessage(connectionId, audioHandleId, audioRequest, null);
    ...
    private VideoRoomResponse createVideoRoom(
        ...
        VideoRoomCreateRequest videoRequest = VideoRoomCreateRequest.create()
        ...
        VideoRoomResponse response = sendVideoRoomPluginMessage(connectionId, videoHandleId, videoRequest, null);
    ...
    private void destroyVideoRoom(
        ...
        VideoRoomDestroyRequest destroyRequest = VideoRoomDestroyRequest.create()
                .request(VideoRoomDestroyRequest.DESTROY)
                .room(videoRoomId)
                .permanent(false);

        VideoRoomResponse response = sendVideoRoomPluginMessage(connectionId, videoHandleId, destroyRequest, null);
    ...
    private void destroyAudioBridgeRoom(
        ...
        AudioBridgeDestroyRequest destroyRequest = AudioBridgeDestroyRequest.create()
                .request(AudioBridgeDestroyRequest.DESTROY)
                .room(audioRoomId)
                .permanent(false);

        AudioBridgeResponse response = sendAudioBridgePluginMessage(connectionId, audioHandleId, destroyRequest, null);
    ...
    public void addMeetingParticipant(
        ...
        Optional<VideoServerSession> videoServerSession = videoServerMeeting.getVideoServerSessions().stream()
        ...
        videoServerSession.ifPresentOrElse(
                session -> videoServerSessionRepository.update(
                        session
                                .userId(userId)
                                .queueId(queueId)
    ...
    private void joinVideoRoomAsPublisher(
            ...
            String videoRoomId,
            MediaType mediaType) {
        VideoRoomResponse videoRoomResponse = sendVideoRoomPluginMessage(
                connectionId,
                videoHandleId,
    ...
    public void destroyMeetingParticipant(String userId, String meetingId) {
        videoServerMeetingRepository
                .getById(meetingId)
                .flatMap(
                        videoServerMeeting -> videoServerMeeting.getVideoServerSessions().stream()
                                .filter(sessionUser -> sessionUser.getUserId().equals(userId))
                                .findFirst())
                .ifPresent(
                        videoServerSession -> {
    ...
    private void muteAudioStream(
        ...
        AudioBridgeResponse audioBridgeResponse = sendAudioBridgePluginMessage(
                meetingConnectionId,
    ...
    public void iceRestartAudio(String userId, String meetingId, String sdp) {
        ...
        iceRestartAudioWithSdp(videoServerSession.getConnectionId(), videoServerSession.getAudioHandleId(), sdp);
    }
    ...
    private void iceRestartVideoWithSdp(
            String connectionId, String videoOutHandleId, String sdp) {
     ...
     *
     * @return true if the video server returns the server_info status, false
     *         otherwise
     * @see <a href=
     *      "https://janus.conf.meetecho.com/docs/rest.html">JanusRestApi</a>
     */
    @Override
    ...
    private VideoServerResponse createConnection() {
        VideoServerMessageRequest request = VideoServerMessageRequest.create()
     ...
     * @param opaqueId     the user id or meeting id associated to this
     *                     handle-session on the VideoServer
     ...
     * @param rtcSessionDescription    the WebRTC negotiation session description
     *                                 (optional)
