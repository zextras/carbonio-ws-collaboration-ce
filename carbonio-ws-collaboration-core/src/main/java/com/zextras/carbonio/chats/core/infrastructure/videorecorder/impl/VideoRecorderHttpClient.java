...
public class VideoRecorderHttpClient implements VideoRecorderClient {
  ...
  private static final String VIDEORECORDER_ROUTING_QUERY_PARAM = "?service_id=%s";
  ...
  @Override
  public void sendVideoRecorderRequest(
      String serverId, String meetingId, VideoRecorderRequest request) {
    try (CloseableHttpResponse response =
        httpClient.sendPost(
            buildVideoRecorderUrl(meetingId)
                + String.format(VIDEORECORDER_ROUTING_QUERY_PARAM, serverId),
            Map.of("Content-Type", "application/json"),
            objectMapper.writeValueAsString(request))) {

      int statusCode = response.getStatusLine().getStatusCode();
      if (statusCode != HttpStatus.SC_OK) {
        throw new VideoServerException("Video recorder returns error response: " + statusCode);
      }
