    ...
    stage("Publishing documentation") {
      ...
      steps {
        dir("dev-guide") {
          ...
          sh '''
            git checkout master
            cp ../carbonio-chats-ce-core-openapi/src/main/resources/chats-core/chats-core-api.yaml ./static/chats/openapi/chats-core-api.yaml
...
boolean hasOpenAPIDocumentChanged() {
  def isChanged = sh(
    script: "git --no-pager show --name-only --pretty=format: | grep -x carbonio-chats-ce-core-openapi/src/main/resources/chats-core/chats-core-api.yaml",
    returnStatus: true
  )
