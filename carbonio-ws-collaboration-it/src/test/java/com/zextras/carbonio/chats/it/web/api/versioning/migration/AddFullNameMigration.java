...
public class AddFullNameMigration implements ApiVersionMigration {
  ...
  public ObjectNode downgrade(ObjectNode input) {
    var value = input.get("fullName").asText();
    String[] parts = value.split(" ");

    input.put("firstName", parts[0]);
    input.put("lastName", parts[1]);
    input.remove("fullName");
    return input;
  }
}
