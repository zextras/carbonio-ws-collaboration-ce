...
public class VideoRecorderExtension
    implements AfterEachCallback, BeforeAllCallback, ParameterResolver {

  private static final String SERVER_HOST = "localhost";
  private static final int SERVER_PORT = 7987;
  private static final Namespace EXTENSION_NAMESPACE =
      Namespace.create(VideoRecorderExtension.class);
  private static final String CLIENT_STORE_ENTRY = "video_recorder_client";
  ...
  public void beforeAll(ExtensionContext context) {
    context
        ...
        .getOrComputeIfAbsent(
            CLIENT_STORE_ENTRY,
            (key) -> {
              ChatsLogger.debug("Starting Video Recorder mock...");
              VideoRecorderMockServer client = new VideoRecorderMockServer(SERVER_PORT);
              InMemoryConfigStore.set(ConfigName.VIDEO_RECORDER_HOST, SERVER_HOST);
              InMemoryConfigStore.set(
                  ConfigName.VIDEO_RECORDER_PORT, Integer.toString(SERVER_PORT));
              return client;
            },
            VideoRecorderMockServer.class);
  }
  ...
  public boolean supportsParameter(
    ...
    return parameterContext.getParameter().getType().equals(VideoRecorderMockServer.class);
  }
  ...
  public Object resolveParameter(
      ParameterContext parameterContext, ExtensionContext extensionContext)
      throws ParameterResolutionException {
    if (parameterContext.getParameter().getType().equals(VideoRecorderMockServer.class)) {
  ...
  public void afterEach(ExtensionContext context) {
    Optional.ofNullable(context.getRoot().getStore(EXTENSION_NAMESPACE).get(CLIENT_STORE_ENTRY))
        .map(mock -> (VideoRecorderMockServer) mock)
